...
class BootstrapIrTests(unittest.TestCase):
    def test_build_ir_from_bootstrap_openapi(self) -> None:
        ...
        inline_keyboard = next(schema for schema in ir["schemas"] if schema["name"] == "InlineKeyboard")
        inline_keyboard_row = next(schema for schema in ir["schemas"] if schema["name"] == "InlineKeyboardRow")
        inline_keyboard_button = next(schema for schema in ir["schemas"] if schema["name"] == "InlineKeyboardButton")
        markup_element_type = next(schema for schema in ir["schemas"] if schema["name"] == "MarkupElementType")
        chat = next(schema for schema in ir["schemas"] if schema["name"] == "Chat")
        share_attachment = next(schema for schema in ir["schemas"] if schema["name"] == "ShareAttachment")
        ...
        self.assertEqual(inline_keyboard["kind"], "array")
        self.assertEqual(inline_keyboard["items"]["kind"], "ref")
        self.assertEqual(inline_keyboard["items"]["ref"], "#/components/schemas/InlineKeyboardRow")
        self.assertEqual(inline_keyboard_row["kind"], "array")
        self.assertEqual(inline_keyboard_row["items"]["kind"], "ref")
        self.assertEqual(inline_keyboard_row["items"]["ref"], "#/components/schemas/InlineKeyboardButton")
        self.assertEqual(inline_keyboard_button["kind"], "union")
        self.assertEqual(len(inline_keyboard_button["variants"]), 6)
        self.assertEqual(inline_keyboard_button["discriminator"]["property_name"], "type")
        self.assertEqual(markup_element_type["kind"], "enum")
        self.assertIn("mention", markup_element_type["enum"])
        dialog_with_user = next(prop for prop in chat["properties"] if prop["name"] == "dialog_with_user")
        pinned_message = next(prop for prop in chat["properties"] if prop["name"] == "pinned_message")
        share_payload = next(prop for prop in share_attachment["properties"] if prop["name"] == "payload")
        self.assertEqual(dialog_with_user["kind"], "ref")
        self.assertEqual(dialog_with_user["ref"], "#/components/schemas/UserWithPhoto")
        self.assertEqual(pinned_message["kind"], "ref")
        self.assertEqual(pinned_message["ref"], "#/components/schemas/Message")
        self.assertEqual(share_payload["kind"], "ref")
        self.assertEqual(share_payload["ref"], "#/components/schemas/ShareAttachmentPayload")
